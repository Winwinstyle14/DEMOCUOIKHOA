...
let grid_row = document.querySelector(".cho");
let listData = JSON.parse(localStorage.getItem("datas"));

let cho = listData.filter(data => data.loai == "cho")
function listDog(datas) {
...
grid_row.innerHTML = cho.map(listDog).join('')
...
ul.addEventListener('click', function (e) {
  ...
  let load = cho.filter(data => data.danhmuc == danhmucuodcchon)
  grid_row.innerHTML = load.map(listDog).join('')
})
